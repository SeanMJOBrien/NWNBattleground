...
//   SendMessageToPC(GetFirstPC(),IntToString(nEvent));
    ...
    else if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
    {
        ...
        int nPoints = GetLocalInt(oItem,"Points");
        if (nPoints == 0) FloatingTextStringOnCreature("This arcane focus has 1 point.",oPC,FALSE);
        else  FloatingTextStringOnCreature("This arcane focus has "+IntToString(nPoints)+" points.",oPC,FALSE);
    }
