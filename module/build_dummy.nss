...
int iHas = GetLocalInt(oMod,sGE+"dummy");
if (iHas==1)
    {
    FloatingTextStringOnCreature("<cþ  >That has already been built.</c>",oPC,0);
    return;
    }
string sWP = "wp_dummy"+sGE;
object oWP = GetWaypointByTag(sWP);
object oBuild = CreateObject(OBJECT_TYPE_PLACEABLE,"special6",GetLocation(oWP));
...
SetLocalInt(oMod,sGE+"dummy",1);
FloatingTextStringOnCreature("<c þ >A dummy is built near the barracks. +1 Score +50 XP.</c>",oPC,0);
}
