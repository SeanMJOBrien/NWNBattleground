...
if (iIron<2)
    {
    FloatingTextStringOnCreature("<cþ  >Requires 2 Iron.</c>",oPC,0);
    return;
    }
int iCopper = GetLocalInt(oMod,sTeam+"cp");
if (iCopper<5)
    {
    FloatingTextStringOnCreature("<cþ  >Requires 5 Copper.</c>",oPC,0);
    return;
    }
...
FloatingTextStringOnCreature("<c þ >A gnomish recyclotron is built near the workshop. +1 Score +50 XP.</c>",oPC,0);
}
