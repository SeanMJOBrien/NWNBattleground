...
int iProp = GetLocalInt(oItem,"evil");
...
if (iType==BASE_ITEM_ARMOR||iType==BASE_ITEM_RING||iType==BASE_ITEM_HELMET||iType==BASE_ITEM_CLOAK)
        {
        itemproperty ipAdd = ItemPropertyDamageResistance(IP_CONST_DAMAGETYPE_DIVINE,iProp+1);
        ...
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Divine Resistance");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"evil",iProp+1);
        }
if (iType == BASE_ITEM_BOOTS && iProp == 0)
        {
        itemproperty ipAdd = ItemPropertyImmunityMisc(IP_CONST_IMMUNITYMISC_DEATH_MAGIC);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Spirit Binding");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"evil",iProp+1);
        }
...
||iType==BASE_ITEM_WARHAMMER||iType==BASE_ITEM_WHIP)
        {
        itemproperty ipAdd = ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_NEGATIVE,iProp+2);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
        ...
        SetLocalInt(oItem,"evil",iProp+2);
        }
if (iType == BASE_ITEM_MAGICSTAFF && iProp == 0)
        {
        itemproperty ipAdd = ItemPropertyCastSpell(IP_CONST_CASTSPELL_NEGATIVE_ENERGY_RAY_3,IP_CONST_CASTSPELL_NUMUSES_3_USES_PER_DAY);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Negative Energy Ray");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"evil",iProp+1);
        }
if (iType==BASE_ITEM_BELT||iType==BASE_ITEM_BRACER)
        ...
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Persuasion");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"evil",iProp+1);
        }
if (iType==BASE_ITEM_LARGESHIELD||iType==BASE_ITEM_SMALLSHIELD||iType==BASE_ITEM_TOWERSHIELD)
        {
        itemproperty ipAdd = ItemPropertyDamageImmunity(IP_CONST_DAMAGETYPE_PIERCING,iProp+1);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Piercing Immunity");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"evil",iProp+1);
        }
...
||iType==BASE_ITEM_SHORTBOW||iType==BASE_ITEM_SLING)
        ...
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Protection");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"evil",iProp+1);
        }
if (iType == BASE_ITEM_AMULET && iProp == 0)
        ...
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Foolhardiness");
            SetLocalInt(oItem,"name",1);
            }
