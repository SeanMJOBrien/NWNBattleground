...
int iProp = GetLocalInt(oItem,"fire");
...
if (iType==BASE_ITEM_ARMOR||iType==BASE_ITEM_RING||iType==BASE_ITEM_HELMET||iType==BASE_ITEM_CLOAK)
        {
        itemproperty ipAdd = ItemPropertyDamageResistance(IP_CONST_DAMAGETYPE_FIRE,iProp+1);
        ...
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Fire Resistance");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"fire",iProp+1);
        }
...
||iType==BASE_ITEM_WARHAMMER||iType==BASE_ITEM_WHIP)
        {
        itemproperty ipAdd = ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_FIRE,iProp+2);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Flames");
            SetLocalInt(oItem,"name",2);
            }
        SetLocalInt(oItem,"fire",iProp+1);
        }
if (iType == BASE_ITEM_MAGICSTAFF && iProp == 0)
        {
        itemproperty ipAdd = ItemPropertyCastSpell(IP_CONST_CASTSPELL_BURNING_HANDS_5,IP_CONST_CASTSPELL_NUMUSES_3_USES_PER_DAY);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Burning Hands");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"fire",iProp+1);
        }
if (iType==BASE_ITEM_BELT||iType==BASE_ITEM_BRACER)
        {
        itemproperty ipAdd = ItemPropertyAbilityBonus(IP_CONST_ABILITY_CON,iProp+1);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Endurance");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"fire",iProp+1);
        }
if (iType==BASE_ITEM_LARGESHIELD||iType==BASE_ITEM_SMALLSHIELD||iType==BASE_ITEM_TOWERSHIELD)
        {
        itemproperty ipAdd = ItemPropertyDamageImmunity(IP_CONST_DAMAGETYPE_FIRE,iProp+1);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Fire Immunity");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"fire",iProp+1);
        }
...
||iType==BASE_ITEM_SHORTBOW||iType==BASE_ITEM_SLING)
        ...
        if (iProp==0)
        {
        itemproperty ipAdd = ItemPropertyUnlimitedAmmo(IP_CONST_UNLIMITEDAMMO_1D6FIRE);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Raining Fire");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"fire",iProp+1);
        }
