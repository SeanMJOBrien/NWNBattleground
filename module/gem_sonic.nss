...
int iProp = GetLocalInt(oItem,"sonic");
...
if (iType==BASE_ITEM_ARMOR||iType==BASE_ITEM_RING||iType==BASE_ITEM_HELMET||iType==BASE_ITEM_CLOAK)
        {
        itemproperty ipAdd = ItemPropertyDamageResistance(IP_CONST_DAMAGETYPE_SONIC,iProp+1);
        ...
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Sonic Resistance");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"sonic",iProp+1);
        }
if (iType == BASE_ITEM_BOOTS && iProp == 0)
        {
        itemproperty ipAdd = ItemPropertyImmunityMisc(IP_CONST_IMMUNITYMISC_PARALYSIS);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Undaunted Movement");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"sonic",iProp+1);
        }
...
||iType==BASE_ITEM_WARHAMMER||iType==BASE_ITEM_WHIP)
        {
        itemproperty ipAdd = ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_SONIC,iProp+2);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Wailing");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"sonic",iProp+2);
        }
if (iType == BASE_ITEM_MAGICSTAFF && iProp == 0)
        {
        itemproperty ipAdd = ItemPropertyCastSpell(IP_CONST_CASTSPELL_SOUND_BURST_3,IP_CONST_CASTSPELL_NUMUSES_3_USES_PER_DAY);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Sound Burst");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"sonic",iProp+1);
        }
if (iType==BASE_ITEM_BELT||iType==BASE_ITEM_BRACER)
        {
        itemproperty ipAdd = ItemPropertyAbilityBonus(IP_CONST_ABILITY_INT,iProp+1);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Logic");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"sonic",iProp+1);
        }
if (iType==BASE_ITEM_LARGESHIELD||iType==BASE_ITEM_SMALLSHIELD||iType==BASE_ITEM_TOWERSHIELD)
        {
        itemproperty ipAdd = ItemPropertyDamageImmunity(IP_CONST_DAMAGETYPE_SONIC,iProp+1);
        IPSafeAddItemProperty(oItem,ipAdd,0.0f,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING);
        FloatingTextStringOnCreature("Gem added successfully to "+sName+".",oPC);
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Sonic Immunity");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"sonic",iProp+1);
        }
...
||iType==BASE_ITEM_SHORTBOW||iType==BASE_ITEM_SLING)
        ...
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Stalwart Defense");
            SetLocalInt(oItem,"name",1);
            }
        SetLocalInt(oItem,"sonic",iProp+1);
        }
if (iType == BASE_ITEM_AMULET && iProp == 0)
        {
        ...
        if (iProp==0&&iName==0)
            {
            SetName(oItem,sName+" of Mind Barrier");
            SetLocalInt(oItem,"name",1);
            }
