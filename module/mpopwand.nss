...
//   SendMessageToPC(GetFirstPC(),IntToString(nEvent));
    ...
    else if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
    {
        ...
        SetLocalInt(oPC,"WPCount",0);
        location lSpot = GetAheadLocation(oPC);
        object oTalkTo = CreateObject(OBJECT_TYPE_PLACEABLE,"ps_mpopwtemp",lSpot);
        AssignCommand(oPC,ActionStartConversation(oTalkTo,"ps_mpopwand",TRUE,FALSE));
    }
