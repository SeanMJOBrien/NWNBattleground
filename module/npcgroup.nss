   ...
   SendMessageToPC(GetFirstPC(),IntToString(nEvent));
    ...
    else if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
    {
        ...
        if (oTarget != OBJECT_INVALID)
            {
            nCount = 1;
            oScan = GetLocalObject(oPC,"GROUP"+IntToString(nCount));
            while (oScan != OBJECT_INVALID)
                {
                if (oScan == oTarget) return;
                nCount ++;
                oScan = GetLocalObject(oPC,"GROUP"+IntToString(nCount));
                }
            SetLocalObject(oPC,"GROUP"+IntToString(nCount),oTarget);
            DeleteLocalInt(oItem,"DClick");
            SendMessageToPC(oPC,GetName(oTarget)+" added. ("+IntToString(nCount)+")");
            }
        else
            {
            if (GetLocalInt(oItem,"DClick") == 0) SetLocalInt(oItem,"DClick",1);
            else
                {
                DeleteLocalInt(oItem,"DClick");
                nCount = 1;
                ...
                while (oScan != OBJECT_INVALID)
                    {
                    DeleteLocalObject(oPC,"GROUP"+IntToString(nCount));
                    nCount ++;
                    oScan = GetLocalObject(oPC,"GROUP"+IntToString(nCount));
                    }
                SendMessageToPC(oPC,"Group cleared.");
                }
            }
    }
