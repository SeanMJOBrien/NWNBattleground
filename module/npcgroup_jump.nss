...
void main()
{
   ...
   SendMessageToPC(GetFirstPC(),IntToString(nEvent));
    ...
    else if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
    {
        ...
        int nCount = 1;
        object oScan = GetLocalObject(oPC,"GROUP"+IntToString(nCount));
        if (oScan == OBJECT_INVALID) SendMessageToPC(oPC,"No group to jump.");
        while ((oScan != OBJECT_INVALID) || (nCount < 13))
            {
            SendCreature(oScan,oPC);
            nCount ++;
