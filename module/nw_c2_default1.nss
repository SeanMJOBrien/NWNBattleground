...
if (GetIsInCombat(OBJECT_SELF)!=TRUE)
    {
    string sMoveto = GetLocalString(OBJECT_SELF,"moveto");
    object oWP = GetWaypointByTag(sMoveto);
    location lDest = GetLocation(oWP);
    ActionForceMoveToLocation(lDest,FALSE);
    string sAttack = GetLocalString(OBJECT_SELF,"attack");
    if (sAttack=="mana")
        {
        ExecuteScript("resume_manapl",OBJECT_SELF);
        return;
        }
    if (sAttack=="nearest")
        {
    ...
    int iRaid = GetLocalInt(OBJECT_SELF,"raider");
    if (iRaid==1)
        {
        object oEnemy = GetNearestEnemy(OBJECT_SELF,d4());
        if (oEnemy==OBJECT_INVALID) GetNearestEnemy(OBJECT_SELF,1);
        if (oEnemy!=OBJECT_INVALID) ActionAttack(oEnemy,FALSE);
        }
    }
