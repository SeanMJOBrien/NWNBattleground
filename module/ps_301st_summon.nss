...
SpeakString("A game of 301 is beginning. "+GetName(GetLocalObject(GetArea(OBJECT_SELF),"oPlayer1"))+" is up.");
SetLocalInt(GetArea(OBJECT_SELF),"iDartGame",1);
if (GetIsPC(GetLocalObject(GetArea(OBJECT_SELF),"oPlayer1")) != TRUE)
    {
    AssignCommand(GetLocalObject(GetArea(OBJECT_SELF),"oPlayer1"),ActionAttack(GetObjectByTag("ps_dartboard")));
    }
}
