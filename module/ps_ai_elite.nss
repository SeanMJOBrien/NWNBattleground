...
************************* [Targeting] *****************************************/
    ...
    if (d6() == 1) SetSpawnInCondition(AI_FLAG_TARGETING_LIKE_MAGE_CLASSES, AI_TARGETING_FLEE_MASTER);
    ...
    SetAIInteger(AI_MELEE_LAST_TO_NEW_TARGET_CHANCE, 50);
    SetAIInteger(AI_RANGED_LAST_TO_NEW_TARGET_CHANCE, 50);
    SetAIInteger(AI_SPELL_LAST_TO_NEW_TARGET_CHANCE, 50);
    ...
    if (d6() <= 5) AI_SetAITargetingValues(TARGETING_MANTALS, TARGET_LOWER, i1, i12);
    else AI_SetAITargetingValues(TARGETING_MANTALS, TARGET_HIGHER, i1, i12);
    // Spell mantals are checked only for the spell target. Either Absense of or got any.
    if (d6() <= 5) AI_SetAITargetingValues(TARGETING_RANGE, TARGET_HIGHER, i2, i9);
        else AI_SetAITargetingValues(TARGETING_RANGE, TARGET_LOWER, i2, i9);
    // Range - very imporant! Basis for all ranged/spell attacks.
    if (d6() <= 5) AI_SetAITargetingValues(TARGETING_AC, TARGET_LOWER, i2, i6);
       else AI_SetAITargetingValues(TARGETING_AC, TARGET_LOWER, i2, i6);
    ...
    if (d6() <= 5) AI_SetAITargetingValues(TARGETING_BAB, TARGET_LOWER, i1, i4);
        else AI_SetAITargetingValues(TARGETING_BAB, TARGET_HIGHER, i1, i4);
    // Hit dice - how powerful in levels the enemy is. Used for all checks.
    if (d6() <= 5) AI_SetAITargetingValues(TARGETING_HITDICE, TARGET_LOWER, i1, i3);
        else AI_SetAITargetingValues(TARGETING_HITDICE, TARGET_HIGHER, i1, i3);
    if (d6() <= 5) AI_SetAITargetingValues(TARGETING_HP_PERCENT, TARGET_LOWER, i1, i3);
    else  AI_SetAITargetingValues(TARGETING_HP_PERCENT, TARGET_HIGHER, i1, i3);
...
************************* [Combat - Fighters] *********************************/
    ...
    SetAIInteger(AI_RANGED_WEAPON_RANGE, 2+d4());
...
************************* [Combat Other - Healers/Healing] ********************/
    ...
    SetAIInteger(AI_HEALING_US_PERCENT, 40+d10(3));
        // % of HP we need to be at until we heal us at all. Default: 50
    SetAIInteger(AI_HEALING_ALLIES_PERCENT, 40+d10(3));
    ...
    SetAIInteger(SECONDS_BETWEEN_STATUS_CHECKS, 18);
