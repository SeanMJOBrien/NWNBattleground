...
if (iEquip<2)
    {
    FloatingTextStringOnCreature("<cþ  >Requires 2 units of Equipment.</c>",oPC,0);
    return;
    }
int iIron = GetLocalInt(oMod,sTeam+"ir");
if (iIron<3)
    {
    FloatingTextStringOnCreature("<cþ  >Requires 3 units of Iron.</c>",oPC,0);
    return;
    }
...
CreateObject(OBJECT_TYPE_CREATURE,"infantry"+sGE,GetLocation(oPC));
