...
if (iMith<1)
    {
    FloatingTextStringOnCreature("<cþ  >Requires 1 unit of Mithril.</c>",oPC,0);
    return;
    }
int iAdam = GetLocalInt(oMod,sTeam+"ad");
if (iAdam<1)
    {
    FloatingTextStringOnCreature("<cþ  >Requires 1 unit of Adamantine.</c>",oPC,0);
    return;
    }
int iSteel = GetLocalInt(oMod,sTeam+"st");
if (iSteel<1)
    {
    FloatingTextStringOnCreature("<cþ  >Requires 1 units of Steel.</c>",oPC,0);
    return;
    }
int iCP = GetLocalInt(oPC,"cp");
if (iCP<25)
    {
    FloatingTextStringOnCreature("<cþ  >That order requires 25 Command Points to carry out.</c>",oPC,0);
    return;
    }
