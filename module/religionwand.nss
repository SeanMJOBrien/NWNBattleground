...
void main()
{
   ...
   SendMessageToPC(GetFirstPC(),IntToString(nEvent));
    ...
    else if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
    {
        ...
        location lSpot = GetAheadLocation(oPC);
        SetLocalObject(oPC,"ReligionTarget",oTarget);
        object oTalkTo = CreateObject(OBJECT_TYPE_PLACEABLE,"ps_religiontemp",lSpot);
        AssignCommand(oPC,ActionStartConversation(oTalkTo,"ps_setreligion",TRUE,FALSE));
    }
