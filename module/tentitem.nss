...
//   SendMessageToPC(GetFirstPC(),IntToString(nEvent));
    ...
    else if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
    {
        ...
        if (!GetIsInCombat(oPC))
            ...
            AssignCommand(oPC,PlaySound("as_sw_clothop1"));
            CreateObject(OBJECT_TYPE_PLACEABLE,"ps_tentplc",GetLocation(oPC));
            DestroyObject(oItem);
            }
        else
            {
            SendMessageToPC(oPC,"You are in combat, now is not the time to make camp.");
            }
    }
