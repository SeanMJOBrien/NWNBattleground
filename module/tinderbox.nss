...
void CreateCamp(location lPlace, object oPC)
{
string sBlueprint = "ps_campfire" + IntToString(d3(1));
object oCampfire = CreateObject(OBJECT_TYPE_PLACEABLE,sBlueprint,lPlace, TRUE);
...
//   SendMessageToPC(GetFirstPC(),IntToString(nEvent));
    ...
    else if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
    {
        ...
        if (GetDistanceBetweenLocations(GetLocation(oPC),lLocation) < 4.0)
            {
            AssignCommand(oPC,ActionPlayAnimation(ANIMATION_LOOPING_GET_LOW,1.0,2.0));
            AssignCommand(oPC,ActionDoCommand(CreateCamp(lLocation,oPC)));
            }
        else
            {
