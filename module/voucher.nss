...
//   SendMessageToPC(GetFirstPC(),IntToString(nEvent));
    ...
    else if (nEvent ==  X2_ITEM_EVENT_ACTIVATE)
    {
        ...
        if (nHours <= 0) SendMessageToPC(oPC,"The time and date scribbled on this voucher has passed, making it mature. Its origin is: "+GetLocalString(oItem,"MerchantID")+".");
        else SendMessageToPC(oPC,"This voucher will mature in another "+SayHours(nHours)+". Its origin is: "+GetLocalString(oItem,"MerchantID")+".");
    }
